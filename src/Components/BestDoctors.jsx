...
const BestDoctors = ({ DoctorsData }) => {
    ...
    useEffect(() => {
        if(showAll) setDisplayData(DoctorsData);
        else setDisplayData(DoctorsData.slice(0, 6));
    }, [DoctorsData, showAll])
        ...
        <div className='bg-[#EFEFEF]'>
            <section className='flex flex-col items-center sm:py-20 px-6 md:px-[40px] py-6 lg:px-[110px]'>
                ...
                <div className='grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12'>
                    {
                        displayData.map((DoctorData) => <DoctorsCard key={DoctorData.id} DoctorData={DoctorData}></DoctorsCard>)
                    }
                </div>
                <button onClick={
                    () => {
